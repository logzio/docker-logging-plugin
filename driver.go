...
func (d *Driver) flushPartialBuffers() {
	...
	for {
		for containerID, lf := range d.idx {
			d.mu.Lock()
			if _, ok := d.idx[containerID]; !ok {
				d.mu.Unlock()
				continue
			}
			delta := time.Now().Sub(lf.logzioLogger.pBuf.startTime)
			if delta > lf.logzioLogger.pBuf.Timeout() {
				buf := lf.logzioLogger.pBuf.Flush()
				logAllMsgs(buf, lf)
			}
			d.mu.Unlock()
		}
...
func validateDriverOpt(loggerInfo logger.Info) (string, error) {
	...
	for opt := range config {
		...
		case logzioFormat, logzioLogSource, logzioTag, logzioToken, logzioType, logzioURL, logzioDirPath,
			envRegex, dockerLabels, dockerEnv, logzioLogAttr,logzioMultiline:
		default:
...
func getMultiline(loggerInfo logger.Info) *Multiline{
	...
	fmt.Printf("config : %v+\n", multilineConfigMap)// todo delete
...
func (logzioLogger *LogzioLogger) multilineMsgs(msg *logger.Message) []logger.Message{
	if logzioLogger.ml == nil{
		return []logger.Message{*msg}
	}
	now := time.Now()
	delta := now.Sub(logzioLogger.ml.StartTime())
	var retMsgs []logger.Message
	if delta >= logzioLogger.ml.timeout {
		mlMsg := logzioLogger.ml.Flush()
		retMsgs = append(retMsgs, mlMsg)
		if currMSg, flush := logzioLogger.ml.Add(msg); flush {
			retMsgs = append(retMsgs, currMSg)
		}
	}else {
		if mlMsg, flush := logzioLogger.ml.Add(msg); flush {
			retMsgs = append(retMsgs, mlMsg)
		}
	}
	return retMsgs
...
func copyBuftoMsg(buf logdriver.LogEntry, lf *ContainerLoggersCtx) logger.Message{
	return logger.Message{
		Line: lf.logzioLogger.pBuf.buf,
		Source: buf.Source,
...
func logAllMsgs(buf logdriver.LogEntry, lf *ContainerLoggersCtx){
	msg := copyBuftoMsg(buf, lf)
	readyToLog := lf.logzioLogger.multilineMsgs(&msg)
	for _, msg := range readyToLog{
	...
	lf.logzioLogger.pBuf.Reset()
	lf.logzioLogger.ml.Reset()
}
...
func consumeLog(lf *ContainerLoggersCtx) {
	...
	ml := lf.logzioLogger.ml
	if ml != nil{
		ml.setStartingTime(time.Now())
	}
	...
	for {
		...
		if len(tBuf) != 0 {
			...
			// Multiline timer check is only after we have a full line or buffer timer timed out
